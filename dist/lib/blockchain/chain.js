...
export class Chain extends AsyncEmitter {
    ...
    async setBestChain(entry, block, prev, flags) {
        ...
        if (entry.hasUnknown(this.network)) {
            this.logger.warning('Unknown version bits in block %d: %s.', entry.height, entry.version.toString(16));
        }
    ...
    async saveAlternate(entry, block, prev, flags) {
        ...
        this.logger.warning('Heads up: Competing chain at height %d:'
            ...
            + ' chaintrust-diff=%s', entry.height, this.tip.height, entry.height, this.tip.hash, entry.hash, this.tip.chaintrust.toString(), entry.chaintrust.toString(), this.tip.chaintrust.sub(entry.chaintrust).toString());
