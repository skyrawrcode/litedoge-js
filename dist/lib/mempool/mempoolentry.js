/*!
 * mempoolentry.js - mempool entry object for ldogejs
...
export class MempoolEntry {
     ...
     */
    getPriority(height) {
        const delta = BigInt(height - this.height);
        const priority = (delta * this.value) / BigInt(this.size);
        let result = this.priority + Number(priority);
        if (result < 0)
            result = 0;
        return result;
    }
     ...
     */
    isFree(height) {
        const priority = BigInt(this.getPriority(height));
    ...
    toRaw() {
        ...
        bw.writeU64BI(this.fee);
        bw.writeU32(this.time);
        bw.writeU64BI(this.value);
        let flags = this.coinbase ? 1 : 0 << 0;
    ...
    fromRaw(data) {
        ...
        this.fee = br.readU64BI();
        this.deltaFee = this.fee;
        this.time = br.readU32();
        this.value = br.readU64BI();
        const flags = br.readU8();
        this.coinbase = !!(flags & 1);
