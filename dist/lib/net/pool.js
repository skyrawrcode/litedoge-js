/*!
 * pool.js - peer management for ldogejs
...
export class Pool extends EventEmitter {
    ...
    sendLocator(locator, peer) {
        ...
        this.logger.info("Requesting blocks from ", locator);
        peer.syncing = true;
        peer.blockTime = Date.now();
        if (this.checkpoints) {
    ...
    getBroadcasted(peer, item) {
        ...
        this.logger.debug('Peer requested %s %h as a %s packet (%s).', item.isTX() ? 'tx' : 'block', item.hash, 'normal', peer.hostname());
    ...
    async _handleHeaders(peer, packet) {
        ...
        for (const header of headers) {
            ...
            //todo:: i can't verify here like this but i should still try something
            if (!header.verify()) {
