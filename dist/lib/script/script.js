            ...
            switch (op.value) {
                ...
                case opcodes.OP_CHECKSIGVERIFY: {
                    ...
                    if (sig.length > 0) {
                        ...
                        const hash = tx.signatureHash(index, subscript, type, version);
    ...
    hasCanonicalPushes() {
        for (const op of this.code) {
            ...
            if (op.value < opcodes.OP_PUSHDATA1 && op.value > opcodes.OP_0 && (op.toLength() === 1 && op.data[0] <= 16))
                ...
                return false;
            if (op.value === opcodes.OP_PUSHDATA1 && op.data.length < opcodes.OP_PUSHDATA1)
                // Could have used a normal n-byte push, rather than OP_PUSHDATA1.
                return false;
            if (op.value === opcodes.OP_PUSHDATA2 && op.data.length <= 0xFF)
                // Could have used an OP_PUSHDATA1.
                return false;
            if (op.value === opcodes.OP_PUSHDATA4 && op.data.length <= 0xFFFF)
