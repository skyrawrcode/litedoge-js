/*!
 * wallet.js - wallet object for ldogejs
...
import { Input, Output, Address, MTX } from '../primitives';
...
export const StakeCombineThreshold = 50n * consensus.COIN;
/**
 * Wallet
...
export class Wallet extends EventEmitter {
    ...
    async createCoinStake(bits, time, searchInterval) {
        ...
        if (balance.confirmed <= this.wdb.reserveBalance) {
            throw new Error("No balance available for coinstake.");
        }
        const targetBalance = balance.confirmed - this.wdb.reserveBalance;
        const unReservedCredits = await this.getUnreservedStakingCoins(targetBalance);
        if (!unReservedCredits || unReservedCredits.length === 0) {
            throw new Error("No balance available for coinstake.");
        }
        ...
        for (const unReservedCredit of unReservedCredits) {
            ...
            const blockFrom = await this.wdb.getEntry(coin.height);
            const prevTx = await this.txdb.getTX(coin.hash);
            // only count coins meeting min age requirement
            if (blockFrom.time + this.network.pos.stakeMinAge > coinStakeTx.time - maxStakeSearchInterval)
                continue;
        ...
        if (reward <= 0)
            return null;
        credit += reward;
        //if it contains a split
        if (coinStakeTx.outputs.length === 3) {
            coinStakeTx[1].value = (credit / 2n / consensus.CENT) * consensus.CENT;
            coinStakeTx[2].value = credit - coinStakeTx.outputs[1].value;
        }
        else {
            coinStakeTx.outputs[1].value = credit;
        }
        //Sign
    ...
    async getUnreservedStakingCoins(targetBalance) {
        ...
        let value = 0n;
        for (const credit of stakableCredits) {
            if (value >= targetBalance)
                break;
            value += credit.coin.value;
            if (credit.coin.value >= targetBalance) {
                // If input value is greater or equal to target then simply insert
                //  it into the current subset and exit
                unReservedStakingCredits.push(credit);
                break;
            }
            else if (credit.coin.value < targetBalance + consensus.CENT) {
                unReservedStakingCredits.push(credit);
                value += credit.coin.value;
            }
        }
        return unReservedStakingCredits;
    }
    /**
     * returns a transactions coin age
     * @param {module:primitives.MTX} transaction
     * @returns {Promise<bigint>}
     */
    async getCoinAge(transaction) {
        ...
        for (const input of transaction.inputs) {
            ...
            const txPrevRecord = await this.txdb.getTX(input.prevout);
            const txPrev = txPrevRecord.tx;
            // previous transaction not in main chain
            if (!txPrevRecord)
                continue;
            if (transaction.time < txPrev.time)
                return -1n; // Transaction timestamp violation
            // Read block header
            const block = await this.getBlock(txPrevRecord.height);
            if (!block)
                return -1n; // unable to read block of previous transaction
            if (block.time + this.network.pos.stakeMinAge > transaction.time)
                continue; // only count coins meeting min age requirement
            const valueIn = txPrev.outputs[input.prevout.index].value;
            centSeconds += valueIn * BigInt(transaction.time - txPrev.time) / consensus.CENT;
        }
    ...
    /**
     * retrieves the Public Key from a coin.
     * @param {module:primitives.Coin} coin
     * @returns {Promise<WalletKey|null>}
     */
    async getCoinSigningKey(coin) {
        const signingAddress = coin.getAddress();
        if (!signingAddress) {
            this.logger.error("Failed to kernel signature");
            return null;
        }
        this.logger.info("Parsed kernel type=%s", signingAddress.getType());
        // only support pay to public key and pay to address
        if (!signingAddress.isPubkeyhash()) {
            this.logger.error("No support for kernel type=%s", signingAddress.getType());
            return null;
        }
        return await this.getPrivateKey(signingAddress);
    }
    /**
     *
     * @param {module:primitives.MTX} coinStakeTx
     * @param {module:wallet.Credit[]}unReservedCredits
     * @param {BigInt} alreadyFunded
     * @param {WalletKey} walletKey
     * @param {BigInt} accountBalance
     * @returns {Promise<BigInt>} Amount funded to the coinstake
     */
    async fundCoinstake(coinStakeTx, unReservedCredits, alreadyFunded, walletKey, accountBalance) {
        let amount = alreadyFunded;
        // Attempt to add more inputs
        // Only add coins of the same key/address as kernel
        for (const unReservedCredit of unReservedCredits) {
            const coin = unReservedCredit.coin;
            if (coinStakeTx.outputs.length === 2 && ((coin.script.equals(walletKey.getScript()) ||
                coin.script.equals(coinStakeTx.outputs[1].script))
                && !coin.hash.equals(coinStakeTx.inputs[0].prevout.hash))) {
                const coinTx = await this.getTX(coin.hash);
                const timeWeight = this.kernel.getStakeWeight(coinTx.time, coinStakeTx.time);
                // Stop adding more inputs if already too many inputs
                if (coinStakeTx.inputs.length >= 100)
                    break;
                // Stop adding more inputs if value is already pretty significant
                if (amount >= StakeCombineThreshold)
                    break;
                // Stop adding inputs if reached reserve limit
                if (amount + coin.value > accountBalance - this.wdb.reserveBalance)
                    break;
                // Do not add additional significant input
                if (coin.value >= StakeCombineThreshold)
                    continue;
                // Do not add input that is still too young
                if (timeWeight < this.kernel.pos.stakeMinAge)
                    continue;
                coinStakeTx.addInput(Input.fromCoin(coin));
                amount += coin.value;
            }
