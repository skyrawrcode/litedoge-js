...
export class ChainDB {
  ...
  async getHeight(hash) {
    ...
    return height.readUInt16LE(0, true);
  }
  ...
  getPrevCache(entry) {
    let prev = this.cacheHash.get(entry.prevBlock);
    return prev || null;
  }
  ...
  async saveGenesis() {
    ...
    this.logger.debug('Creating genesis block from hex')
    this.logger.debug(genesis)
    const block = Block.fromRaw(genesis, 'hex');
    const entry = ChainEntry.fromBlock(block, null);
  ...
  async verifyFlags(state:ChainState):Promise<void> {
    ...
    if (needsSave) {
      this.logger.info('Rewriting chain flags.');
      await this.saveFlags();
    }

    if (needsPrune) {
      this.logger.info('Retroactively pruning chain.');
  ...
  async _removeChain(hash) {
    ...
    for (; ;) {
      ...
      // on successful write.
      this.cacheHash.unpush(tip.hash.toString());
