/*!
 * private.js - hd private keys for ldogejs
...
export interface HDPrivateKeyOptions {
  depth: number;
...
export class HDPrivateKey {
  ...
  constructor(options?: HDPrivateKeyOptions) {
    this.depth = 0;
  ...
  fromOptions(options: HDPrivateKeyOptions) {
  ...
  fromKey(key: Buffer, entropy: Buffer) {
  ...
  fromRaw(data: Buffer, network: (Network | NetworkType) | null) {
