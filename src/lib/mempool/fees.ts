/*!
 * fees.js - fee estimation for ldogejs
...
const MIN_FEERATE = 1000000n; // 0.01 coins
const MAX_FEERATE = BigInt(1e6); /* 1e7 */
const INF_FEERATE = consensus.MAX_MONEY;
const MIN_PRIORITY = BigInt(10);
const MAX_PRIORITY = BigInt(1e16);
const INF_PRIORITY = BigInt(1e9) * consensus.MAX_MONEY;
const FEE_SPACING = 10n;
const PRI_SPACING = 2n;
...
class ConfirmStats {
  ...
  init(buckets, maxConfirms, decay) {
    ...
    this.buckets = new BigInt64Array(buckets.length);
    this.bucketMap = new DoubleMap();

    for (let i = 0; i < buckets.length; i++) {
      this.buckets[i] = BigInt(buckets[i]);
