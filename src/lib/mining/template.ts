/*!
 * template.js - block template object for ldogejs
...
export class BlockTemplate {
  ...
  fromOptions(options) {
    ...
    if (options.fees != null) {
      assert(typeof options.fees === 'bigint');
  ...
  /**
   * Calculate the proof of work block reward.
   * @returns {Amount}
   */

  getProofOfWorkReward() {
    const reward = consensus.getProofOfWorkReward(this.height);
    return reward + this.fees;
  }

  /**
   * Calculate the proof of stake block reward.
   * @returns {Amount}
   */

  getProofOfStakeReward() {
    const reward = consensus.getProofOfStakeReward(this.height);
    return reward + this.fees;
  }
  ...
  createCoinbase() {
    ...
    //   CB size = 125
    //   Sigops cost = 4
    assert.strictEqual(cb.getWeight() + padding, 104);
    assert.strictEqual(cb.getBaseSize() + padding, 104);
    assert.strictEqual(cb.getSize() + padding, 104);
