/*!
 * rpc.js - bitcoind-compatible json rpc for ldogejs.
...
export class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getstakinginfo', this.getStakingInfo);
  ...
  async getBlockchainInfo(args, help) {
    ...
    return {
      ...
      chaintrust: this.chain.tip.chaintrust.toString('hex', 64),
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      sigoplimit: consensus.MAX_BLOCK_SIGOPS_COST | 0,
  ...
  async getStakingInfo(args, help) {
    return this.getMiningInfo(args, help);
  }
  ...
  async getHashRate(lookup, height?:number) {
    ...
    const work = tip.chaintrust.sub(entry.chaintrust);
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      chaintrust: entry.chaintrust.toString('hex', 64),
