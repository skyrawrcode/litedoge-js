/*!
 * address.js - address object for ldogejs
...
export const ZERO_HASH160 = Buffer.alloc(20, 0x00);
...
export class Address {
  ...
  constructor(options?: AddressOptions, network?) {
    this.type = AddressTypes.PUBKEYHASH;
    this.version = -1;
  ...
  getPrefix(network: Network) {
    network = Network.get(network);

    const prefixes = network.addressPrefix;

    switch (this.type) {
      case AddressTypes.PUBKEYHASH:
        return prefixes.pubkeyhash;
      case AddressTypes.SCRIPTHASH:
  ...
  fromString(addr, network): Address {
  ...
  fromBech32(addr: any, network: any): Address {
  ...
  fromScript(script:Script) {
    ...
    if (sh) {
      this.hash = sh;
      this.type = AddressTypes.SCRIPTHASH;
      this.version = -1;
      return this;
    }
  ...
  fromHash(hash, type, version) {
    if (typeof type === 'string') {
      type = AddressTypes[type.toUpperCase()];
      assert(type != null, 'Not a valid address type.');
    }

    if (type == null)
      type = AddressTypes.PUBKEYHASH;
  ...
  fromPubkeyhash(hash) {
    const type = AddressTypes.PUBKEYHASH;
  ...
  isPubkeyhash() {
    return this.type === AddressTypes.PUBKEYHASH;
  }
