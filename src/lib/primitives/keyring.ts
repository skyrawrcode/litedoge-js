/*!
 * keyring.js - keyring object for ldogejs
...
export class KeyRing implements KeyRingOptions {
  ...
  fromKey(key: Buffer, compress?: boolean | null) {
  ...
  /**
   * Get scripthash.
   * @param enc - `"hex"` or `null`.
   * @returns
   */

  getScriptHash(enc?: 'hex' | null) {
  ...
  getScriptAddress(enc: 'base58' | 'string', network: Network) {
    ...
    if (!this._scriptAddress) {
      ...
      this._scriptAddress = Address.fromScripthash(hash);
    }
