/*!
 * mtx.js - mutable transaction object for ldogejs
...
export class MTX extends TX {
  ...
  addOutpoint(outpoint: Outpoint | object): Input {
  ...
  async signInputAsync(index: number, coin: Coin | Output, ring: KeyRing, type: SighashType | null, pool: WorkerPool | null): Promise<any> {
    if (!pool)
  ...
  setSequence(index: number, locktime: number, seconds: boolean | null) {
    const input = this.inputs[index];
