...
export class TX {
  ...
  signatureHash(index: number, prev: Script, type: SighashType, version: number): Buffer {
  ...
  checkInput(index: number, coin: Coin | Output, flags: VerifyFlags) {
    const input = this.inputs[index];
  ...
  async checkInputAsync(index: number, coin: Coin | Output, flags: VerifyFlags, pool: WorkerPool | null): Promise<any> {
    const input = this.inputs[index];
  ...
  async verifyInputAsync(index: number, coin: Coin | Output, flags: VerifyFlags, pool: WorkerPool | null): Promise<any> {
    try {
  ...
  _getAddresses(view: CoinView): Array<any> {
  ...
  checkStandard(): Array<any> {
    ...
    for (const output of this.outputs) {
      ...
      if (!output.script.hasCanonicalPushes()) {
        return [false, 'scriptpubkey-non-canonical-push', 0]
      }
  ...
  checkInputs(view: CoinView, height: number, limitFree: boolean = true): Array<any> {
  ...
  getChainValue(view: CoinView): BigInt {
    if (this.isCoinbase())
      return 0n;
