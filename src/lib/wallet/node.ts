/*!
 * server.js - wallet server for ldogejs
...
export class WalletNode extends Node {
  ...
  constructor(options) {
    super('ldogejs', 'wallet.conf', 'wallet.log', options);
    ...
    this.staker = new Staker({
      staking: this.config.bool('staking', true),
      reserveBalance: BigInt(this.config.uint('reserve-balance', 0)),
    });
  ...
  init() {
    ...
    this.staker.on('error', err => this.error(err));
    this.loadPlugins();
  }
