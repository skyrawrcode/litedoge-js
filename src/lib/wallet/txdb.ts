...
export class TXDB {
   ...
   * Get transaction.
   * @param {Hash} hash
   * @returns {Promise<TxRecord>} - Returns {@link TX}.
   */
...
export class Balance {
  ...
  applyTo(balance) {
    ...
    assert(balance.tx >= 0);
    assert(balance.coin >= 0n);
    assert(balance.unconfirmed >= 0n);
    assert(balance.confirmed >= 0n);
  }
  ...
  toRaw() {
    ...
    bw.writeU64BI(this.unconfirmed);
    bw.writeU64BI(this.confirmed);
  ...
  toJSON(minimal) {
    return {
      ...
      tx: this.tx,
      coin: this.coin,
      unconfirmed: Number(this.unconfirmed),
 ...
 * @property {module:primitives.Coin} coin
