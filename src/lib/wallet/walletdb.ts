...
export class WalletDB extends EventEmitter {
  ...
  _bind() {
    ...
    this.client.bind('tx', async (tx: TX) => {
      try {
        await this.addTX(tx);
      } catch (e) {
        this.emit('error', e);
  ...
  async addTX(tx: TX, block?: records.BlockMeta) {
    const unlock = await this.txLock.lock();
    try {
...
class WalletOptions {
  ...
  fromOptions(options: WalletOptionsOptions) {
    ...
    if (options.reserveBalance != null) {
      assert(typeof options.reserveBalance === 'bigint');
      this.reserveBalance = options.reserveBalance;
    }
